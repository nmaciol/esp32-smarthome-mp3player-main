...
void mqttCallback(char* topic, byte* message, unsigned int length) {
  ...
  } else if (String(topic) ==  S_HOST_NAME + "/ls" or  String(topic) == S_MQTT_HOUSE + "/ls") {
...
void mqttReconnect() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect((S_HOST_NAME + "--mp3Player").c_str(), S_MQTT_USER.c_str(), S_MQTT_PASSWORD.c_str())) {
        ...
        mqttClient.subscribe((S_HOST_NAME + "/ls").c_str());
        ...
        mqttClient.subscribe((S_MQTT_HOUSE + "/ls").c_str());
        // Startup
        delay (100);
        if(bStartup){
  ...
  if (!copier.copy()  or copier.copy() == 0)  {
    ...
    if(currentLiveStream.length() > 0  and queueOrder.isEmpty()){
      ...
      if(!url.begin(currentLiveStream.c_str(),"audio/mp3")){
        ...
        //Eintragen deines Wunschsenders      
